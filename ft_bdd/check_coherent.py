...
def create_minimalcutset(a, cache=None):
    ...
    if a in cache:
        return cache[a]
    if a.low.isValue() is True and a.high.isValue() is True:
        result = _check_TT(a, cache)
    if a.low.isValue() is True and a.high.isValue() is False:
        result = _check_TF(a, cache)
    if a.low.isValue() is False and a.high.isValue() is True:
        result = _check_FT(a, cache)
    if a.low.isValue() is False and a.high.isValue() is False:
...
def _check_TT(a, cache):
    ...
    if a.low.value is True and a.high.value is False:
        return False
    else:
        p = []
        p.append([a.var])
