...
class BinOperator:
    ...
    def apply(self, bdd, f, g):
        ...
        if (f, g) in cache:
            return cache[(f, g)]
        if f.isValue() is True and g.isValue() is True:
            result = self.applyTT(bdd, f, g)
        elif f.isValue() is True and g.isValue() is False:
            result = self.applyTN(bdd, f, g)
        elif f.isValue() is False and g.isValue() is True:
...
class GraphvizVisitor(Visitor):
    ...
    def todot(self, bdd, f):
        ...
        self.str.write('digraph { layout=dot; overlap=false;'
                       ' splines=true; node[fontsize=10];\n')
        self.visit(bdd, f)
        self.str.write('}')
    ...
    def visitT(self, bdd, f):
        if f.value is True:
            self.str.write('"obj{term}" [shape = square,'
                           ' label = "故障"];\n'.format(term=id(f)))
        else:
            self.str.write('"obj{term}" [shape = square,'
                           ' label = "正常"];\n'.format(term=id(f)))
